...
/**
 * Generate synthetic or simulated daily trading data based on strategy and date range
 */
function generateBacktestSimulation(strategyId, symbol = 'BANKNIFTY', days = 22, capital = 100000) {
  ...
  tradingDates.forEach((dateObj, idx) => {
    ...
    // Calculate daily PnL (65% win probability characteristic of institutional strangle/breakout)
    ...
    const dayTradesCount = 2; // CE + PE legs
    const dayTrades = [];
    ...
    const ceTrade = {
      ...
      netPnl: cePnl,
      exitReason: isWin ? 'Square Off 15:10' : 'SL Triggered (1% Leg SL)',
      status: cePnl >= 0 ? 'WIN' : 'LOSS',
      spotRefPrice: basePrice + Math.round((pseudoRand() - 0.5) * 200),
      fillModel: 'Next 5m Candle Open + 0.05% Slippage',
    ...
    const peTrade = {
      ...
      netPnl: pePnl,
      exitReason: isWin ? 'Square Off 15:10' : 'Trailing SL Lock Hit',
  ...
  return {
    ...
    provenance: {
      ...
      executionModel: 'Next 5m Candle Open + 0.05% Slippage'
    },
    summary,
    dailyPnlBars,
...
router.post('/run', (req, res) => {
  try {
    const {
      strategyId = '1_percent_sl_strangle_bnf',
      symbol = 'BANKNIFTY',
      days = 22,
      capital = 100000
    } = req.body || {};
    ...
    res.status(500).json({
      success: false,
      message: error.message || 'Error executing backtest simulator.'
    });
  }
});
...
const handleExport = (req, res) => {
  try {
    const strategyId = req.query.strategyId || req.body?.strategyId || '1_percent_sl_strangle_bnf';
    const symbol = req.query.symbol || req.body?.symbol || 'BANKNIFTY';
